...
class DatabaseService:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        # Clean up stale investigations on startup
        self._cleanup_stale_investigations()
    ...
    def _cleanup_stale_investigations(self):
        """Clean up stale investigations on startup"""
        with self.get_session() as session:
            # Delete all investigations that are not completed
            # This ensures we start fresh on each restart
            stale_investigations = session.query(Investigation).filter(
                Investigation.status.notin_(['completed', 'failed'])
            ).all()

            for inv in stale_investigations:
                # Delete associated timeline events first
                session.query(InvestigationTimeline).filter_by(
            ...
            if stale_investigations:
                print(f"🧹 Cleaned up {len(stale_investigations)} stale investigations on startup")
