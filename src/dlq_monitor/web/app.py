...
class MCPService:
    ...
    def __init__(self):
        ...
        self._cache_ttl = timedelta(seconds=5)  # Reduced cache to 5 seconds for better responsiveness
        ...
        try:
            ...
            if "QueueUrls" in response:
                for queue_url in response["QueueUrls"]:
                    ...
                    attrs = self.sqs_client.get_queue_attributes(QueueUrl=queue_url, AttributeNames=["All"])[
                        "Attributes"
                    ]
                ...
                try:
                    ...
                    queue_data = {
                        ...
                        "status": (
                            "critical"
                            if messages > 100
                            else "warning" if messages > 10 else "alert" if messages > 0 else "ok"
                        ),
        ...
        try:
            ...
            for stream in response.get("logStreams", []):
                events = self.cloudwatch_client.get_log_events(
                    logGroupName=log_group, logStreamName=stream["logStreamName"], limit=limit // 5
                )
...
def get_dlq_messages(queue_name):
    ...
    try:
        ...
        response = mcp_service.sqs_client.receive_message(
            QueueUrl=queue["url"], MaxNumberOfMessages=10, VisibilityTimeout=0, WaitTimeSeconds=0
        )
...
def get_logs():
    ...
    try:
        ...
        return jsonify({"logs": "\n".join(logs), "timestamp": datetime.now().isoformat()})
    except Exception as e:
        logger.error(f"Error fetching logs: {e}")
        return jsonify({"logs": f"Error fetching logs: {str(e)}", "timestamp": datetime.now().isoformat()}), 500
...
def start_investigation():
    ...
    try:
        ...
        return jsonify({"investigation_id": investigation_id, "status": "started", "pid": process.pid})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        for queue_name in queues:
            try:
                ...
                cmd = [sys.executable, str(manual_investigation_path), queue_name]
                ...
                results.append(
                    {"queue": queue_name, "investigation_id": investigation_id, "status": "started", "pid": process.pid}
                )
                ...
                results.append({"queue": queue_name, "status": "failed", "error": str(e)})
...
def execute_terminal_command():
    """Execute terminal commands and natural language queries"""
    try:
        data = request.json
...
Natural Language:
  ...
  - "What's happening with the payment queue?"
  ...
  - "Are there any errors?" """
            ...
            total_messages = sum(q.get("messages", 0) for q in dlqs)

            response = f"""System Status: OPERATIONAL
Active Agents: {active_agents}
DLQ Messages: {total_messages}
Investigations: {len(investigation_tracker.get_active_investigations())} active, 5 completed today
Last Check: {datetime.now().strftime('%I:%M:%S %p')}"""
            ...
            for agent_id, agent_data in agents.items():
                status_emoji = "✅" if agent_data["status"] == "active" else "⚪"
                agent_lines.append(f"{status_emoji} {agent_data['name']} - {agent_data['status'].upper()}")
                if agent_data.get("currentTask"):
                    agent_lines.append(f"   └─ {agent_data['currentTask']}")

            response = "Active Agents:\n" + "\n".join(agent_lines) if agent_lines else "No agents active"
            return jsonify({"response": response, "type": "response"})

        elif command_lower == "dlq" or command_lower == "dlqs":
            # Get real DLQ status
            dlqs = mcp_service.get_dlq_queues()
            if dlqs:
                dlq_lines = []
                for dlq in dlqs:
                    if dlq.get("messages", 0) > 0:
                        dlq_lines.append(f"🔴 {dlq['name']}: {dlq['messages']} messages")
                    else:
                        dlq_lines.append(f"✅ {dlq['name']}: No messages")
                response = "DLQ Status:\n" + "\n".join(dlq_lines)
            else:
                response = "No DLQ queues found"
            return jsonify({"response": response, "type": "response"})

        elif command_lower.startswith("investigate"):
            # Parse DLQ name from command
            parts = command.split()
            if len(parts) > 1:
                dlq_name = " ".join(parts[1:])
                # Trigger investigation
                response = f"Starting investigation for {dlq_name}..."
                # Here you would actually trigger the investigation
                return jsonify({"response": response, "type": "info"})
            else:
                return jsonify(
                    {"response": "Please specify a DLQ to investigate: investigate <dlq-name>", "type": "error"}
                )
            ...
            log_entries = []
            try:
                log_path = Path.home() / "LPD Repos" / "lpd-claude-code-monitor" / "logs" / "dlq_monitor.log"
                if log_path.exists():
                    with open(log_path) as f:
                ...
                response = "Recent Logs:\n" + "\n".join(log_entries) if log_entries else "No recent logs"
            except Exception as e:
                response = f"Error reading logs: {e}"
            return jsonify({"response": response, "type": "response"})

        elif command_lower == "clear":
            return jsonify({"response": "clear", "type": "clear"})

        # Natural language processing
        elif "payment" in command_lower or "queue" in command_lower:
            dlqs = mcp_service.get_dlq_queues()
            payment_dlqs = [q for q in dlqs if "payment" in q["name"].lower()]
            if payment_dlqs:
                msgs = sum(q.get("messages", 0) for q in payment_dlqs)
                response = (
                    f"Payment queue status: {msgs} messages in DLQ"
                    if msgs > 0
                    else "Payment queue is clear - no messages in DLQ"
                )
            else:
                response = "No payment queues found in the system"
            return jsonify({"response": response, "type": "response"})

        elif "investigation" in command_lower:
            investigations = investigation_tracker.get_active_investigations()
            if investigations:
                response = f"Active investigations: {len(investigations)}\nMost recent: {investigations[0].get('dlq_name', 'Unknown')}"
            else:
                response = "No active investigations at the moment"
            return jsonify({"response": response, "type": "response"})
            ...
            try:
                ...
                response = "Unable to check for errors"
            return jsonify({"response": response, "type": "response"})
...
def get_agent_activities():
    """Get real-time agent activities for Glass Room panel"""
    try:
        activities = []
        ...
        # Check ADK monitor log for recent activities
        adk_log_path = Path.home() / "LPD Repos" / "lpd-claude-code-monitor" / "logs" / "adk_monitor.log"
        if adk_log_path.exists():
            with open(adk_log_path) as f:
                lines = f.readlines()[-100:]  # Last 100 lines

                for line in lines:
                    timestamp_str = line[:19] if len(line) > 19 else None
                    if timestamp_str:
                        try:
                            # Parse activities from log
                            if "Investigation Agent" in line:
                                if "investigating" in line.lower():
                                    details = ""
                                    if "DLQ:" in line:
                                        dlq_name = line.split("DLQ:")[1].strip().split()[0]
                                        details = f"Analyzing {dlq_name}"
                                    activities.append(
                                        {
                                            "id": f"inv_{len(activities)}",
                                            "timestamp": timestamp_str,
                                            "agent": "DLQ Investigator",
                                            "action": "Investigating DLQ messages",
                                            "status": "running",
                                            "details": details,
                                        }
                                    )
                                ...
                                activities.append(
                                    {
                                        "id": f"ana_{len(activities)}",
                                        "timestamp": timestamp_str,
                                        "agent": "Code Analyzer",
                                        "action": "Analyzing error patterns",
                                        "status": "running",
                                        "details": line.split("analyzing")[1].strip() if "analyzing" in line else "",
                                    }
                                )
                            elif "Code Debugger" in line and "debugging" in line.lower():
                                activities.append(
                                    {
                                        "id": f"dbg_{len(activities)}",
                                        "timestamp": timestamp_str,
                                        "agent": "Code Debugger",
                                        "action": "Debugging code issues",
                                        "status": "running",
                                        "details": "",
                                    }
                                )
                            elif "PR" in line and ("created" in line.lower() or "creating" in line.lower()):
                                pr_num = ""
                                if "#" in line:
                                    pr_num = line.split("#")[1].split()[0] if len(line.split("#")) > 1 else ""
                                activities.append(
                                    {
                                        "id": f"pr_{len(activities)}",
                                        "timestamp": timestamp_str,
                                        "agent": "PR Creator",
                                        "action": f"Created PR #{pr_num}" if pr_num else "Creating pull request",
                                        "status": "completed" if "created" in line.lower() else "running",
                                        "details": "Fix implementation ready for review",
                                    }
                                )
                            elif "completed" in line.lower() or "finished" in line.lower():
                                activities.append(
                                    {
                                        "id": f"cmp_{len(activities)}",
                                        "timestamp": timestamp_str,
        ...
        # Add current agent states as activities
        for agent_id, agent_data in agents.items():
            if agent_data["status"] != "idle":
                activities.append(
                    {
                        "id": f"{agent_id}_current",
                        "timestamp": datetime.now().isoformat(),
                        "agent": agent_data["name"],
                        "action": agent_data.get("currentTask", "Processing"),
                        "status": "running" if agent_data["status"] == "active" else agent_data["status"],
                        "details": "",
                    }
                )

        # If no activities found, add some recent monitoring activities as placeholders
        if len(activities) == 0:
            # Check recent monitoring status
            activities.append(
                {
                    "id": "mon_status",
                    "timestamp": datetime.now().isoformat(),
                    "agent": "DLQ Monitor",
                    "action": "Monitoring all queues",
                    "status": "running",
                    "details": "Last check: " + datetime.now().strftime("%H:%M:%S"),
                }
            )
                ...
                activities.append(
                    {
                        ...
                        "agent": agent_data["name"],
                        "action": "Standing by",
                        "status": "completed" if agent_data["status"] == "idle" else "running",
                        "details": "Ready for investigations",
                    }
                )
...
def handle_start_investigation(data):
    ...
    with open(trigger_file, "w") as f:
        json.dump(
            {"dlq": dlq_name, "messages": messages, "timestamp": datetime.now().isoformat(), "source": "web_dashboard"},
            f,
        )

    emit("alert", {"message": f"Investigation triggered for {dlq_name}", "type": "success"})
                ...
                "queues": [
                    {
                        ...
                        "status": (
                            "critical"
                            if queue.get("messages", 0) > 100
                            else "warning" if queue.get("messages", 0) > 50 else "ok"
                        ),
                    }
                    for queue in queues
                ]
...
def handle_get_logs(data):
    ...
    try:
        ...
        if os.path.exists(log_file):
            ...
            alt_locations = ["dlq_monitor_FABIO-PROD_sa-east-1.log", "adk_monitor.log", "web_dashboard.log"]
        ...
        logger.error(f"Error fetching logs: {e}")
        emit("logs_data", {"logs": f"Error fetching logs: {str(e)}", "timestamp": datetime.now().isoformat()})
    ...
    if investigation_service:
        ...
        if dlq:
            ...
            investigation_id = investigation_service.start_investigation(
                dlq_name=dlq_name, message_count=dlq["messages"]
            )
...
def get_agent_status():
    ...
    agents = {
        "investigator": {"name": "Investigation Agent", "status": "idle", "lastActivity": None, "currentTask": None},
        "analyzer": {"name": "DLQ Analyzer", "status": "idle", "lastActivity": None, "currentTask": None},
        "debugger": {"name": "Code Debugger", "status": "idle", "lastActivity": None, "currentTask": None},
        "reviewer": {"name": "Code Reviewer", "status": "idle", "lastActivity": None, "currentTask": None},
    }
...
def get_system_stats():
    ...
    try:
        ...
        today_investigations = sum(
            1 for inv in sessions.values() if datetime.fromisoformat(inv["start_time"]).date() == today
        )
        ...
        return {"messagesProcessed": 0, "investigationsToday": 0, "prsCreated": 0, "issuesResolved": 0}
...
if __name__ == "__main__":
    ...
    socketio.run(app, debug=False, host="0.0.0.0", port=port, allow_unsafe_werkzeug=True, log_output=False)
